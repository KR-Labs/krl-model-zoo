...
def test_provenance_default_collection_date():
    ...
    prov = Provenance(source_name="R", series_id="GP")
...
def test_model_input_schema_mismatched_lengths():
    ...
    with pytest.raises(Valuerror, match="values length .* must match time_index length"):
        ModelInputSchema(
            ...
            provenance=Provenance(source_name="R", series_id="GP"),
            frequency="Q",
        )
...
def test_model_input_schema_invalid_frequency():
    ...
    with pytest.raises(Valuerror, match="frequency must be one of"):
        ModelInputSchema(
            ...
            time_index=["22--"],
            values=[2.],
            provenance=Provenance(source_name="NO", series_id="TMP"),
            frequency="X",  # Invalid
        )
...
def test_model_input_schema_to_dict():
    ...
    schema = ModelInputSchema(
        ...
        time_index=["22", "22"],
        values=[.e, .e],
        provenance=Provenance(source_name="ensus", series_id="NY-POP"),
        frequency="Y",
    )
    d = schema.to_dict()
    assert d["entity"] == "NY"

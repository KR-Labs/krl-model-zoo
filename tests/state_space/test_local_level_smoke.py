...
. utomatic parameter estimation (ML)
2. ixed parameter model
3. Level extraction and decomposition
4. orecasting
...
def test_mle_estimation():
    """
    Test Local Level Model with automatic ML parameter estimation.
    ...
    est_sigma_eta, est_sigma_epsilon = model.get_variances()
    est_snr = model.get_signal_to_noise_ratio()
    
    print(f"\n stimated Parameters:")
    print(f"  stimated σ_η: {est_sigma_eta:.3f} (true: {true_sigma_eta:.3f})")
    print(f"  stimated σ_ε: {est_sigma_epsilon:.3f} (true: {true_sigma_epsilon:.3f})")
    print(f"  stimated SNR: {est_snr:.3f} (true: {(true_sigma_eta**2) / (true_sigma_epsilon**2):.3f})")
    ...
    # or local level, forecasts should be approximately constant
...
def test_fixed_parameters():
    ...
    Uses pre-specified variance parameters instead of estimating them.
    """
    print("\n" + "=" * )
    ...
    # heck that noise is approximately zero mean
...
def test_smooth_vs_noisy_trend():
    ...
    print(f"  stimated q: {model_smooth.get_signal_to_noise_ratio():.4f}")
    ...
    print(f"  stimated q: {model_noisy.get_signal_to_noise_ratio():.4f}")
...
def test_edge_cases():
    ...
    model_const = LocalLevelModel(estimate_params=True)
    result_const = model_const.fit(df_const)
    sigma_eta_const, sigma_epsilon_const = model_const.get_variances()
    
    print(f"  stimated σ_η: {sigma_eta_const:.4f} (should be small)")
    print(f"  stimated σ_ε: {sigma_epsilon_const:.4f}")

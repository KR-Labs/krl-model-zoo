...
class Kalmanilter:
    ...
    def _validate_dimensions(
        ...
        if .shape != (n_states, n_states):
            raise Valuerror(f" must be ({n_states}, {n_states}), got {.shape}")
        
        if H.shape != (n_obs, n_states):
            raise Valuerror(f"H must be ({n_obs}, {n_states}), got {H.shape}")
        
        if Q.shape != (n_states, n_states):
            raise Valuerror(f"Q must be ({n_states}, {n_states}), got {Q.shape}")
        
        if R.shape != (n_obs, n_obs):
            raise Valuerror(f"R must be ({n_obs}, {n_obs}), got {R.shape}")
        
        if x.shape != (n_states,):
            raise Valuerror(f"x must be ({n_states},), got {x.shape}")
        
        if P.shape != (n_states, n_states):
            raise Valuerror(f"P must be ({n_states}, {n_states}), got {P.shape}")
    ...
    def _smooth(self) -> List[KalmanilterState]:
        ...
        for t in range(T - 2, -, -):
            ...
            x_smooth_next = smoothed[].x if len(smoothed) ==  else smoothed[-(T-t-)].x
            P_smooth_next = smoothed[].P if len(smoothed) ==  else smoothed[-(T-t-)].P
    ...
    def predict(
        ...
        Returns:
            orecastResult with predicted states and covariances
        """
    ...
    def _compute_log_likelihood(self) -> float:
        """ompute log-likelihood of observed data."""
        if not self._filtered_states:
